...
// #define TINYOBJLOADER_200_TEST 1
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable: 4244 4305 4804 4267)
#endif // _MSC_VER
...
static bool LoadObjAndConvert(float bmin[3], float bmax[3],
  ...
  std::string base_dir = utils::extract_file_directory_t<std::string>(filename); // GetBaseDir(filename);
  if (base_dir.empty()) {
    base_dir = ".";
  }
#ifdef _WIN32
    ...
    for (size_t m = 0; m < materials.size(); m++) {
      ...
      if (mp->diffuse_texname.length() > 0) {
        ...
        if (textures.find(mp->diffuse_texname) == textures.end()) {
          ...
          if (!utils::is_file_exist(texture_filename)) {
            // Append base dir.
